...
for ID in $(gh api \
  ...
  # Aktifse pasife çek (hata almamak için 2>/dev/null eklendi)
  gh api --method POST -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" /repos/$OWNER/$REPO/deployments/$ID/statuses -f state="inactive" 2>/dev/null
