...
- gparted
  ...
  - dnf if you are on live-iso
  - gnome-disk-utility does not show long partition names + and not show flags
  - Features
    - On README file of https://gitlab.gnome.org/GNOME/gparted They mention that you must have some commands on OS to run all features.
    - I have a nix command which installs all command as nix.
    - GParted Live ISO includes all commands.
    - All features can be checked online: https://gparted.org/features.php
...
- org.gimp.GIMP
  - flatpak
  - has object removal feature

- org.kde.krita
  - flatpak
  - has object removal feature
    
    sol menüde "Smart Patch Tool" var. o seçilir. sonra silinmesi gerekn obje seçilir. fare tıklanması bırakıldığı anda işlem gerçekleşir. sağ taraftaki menüde 2 ayar var:
    - Accuracy: yüksek olması daha iyi.
    - Patch size: yükske veya düşük değer denenmeli.

## Media Others
- org.kde.kid3
  - flatpak
  - remove opus metadata (which can not be removed via ExifTool)
- org.kde.kdenlive
  - flatpak

## Screenshot fast edit
- com.github.maoschanz.drawing
  - flatpak
  - fast & simple image editor only for basic operations.
- be.alexandervanhee.gradia
  - flatpak
  - ocr and very simple paint

## QR
- `org.kde.qrca`
  - `flatpak`
  - scan QR from camera or photo file
...
# copy flatpak file as "firefox-flatpak" name.
cp /var/lib/flatpak/exports/share/applications/org.mozilla.firefox.desktop $HOME/.local/share/applications/firefox-flatpak.desktop

# change manually the content of the file:
# Name: Firefox_native
gnome-text-editor $HOME/.local/share/applications/firefox-native.desktop

# change manually the content of the file:
# Name: Firefox_flatpak
gnome-text-editor $HOME/.local/share/applications/firefox-flatpak.desktop
