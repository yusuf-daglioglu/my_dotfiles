...
- if linux OS and its device then choose BTRFS.
  ...
  note: do not create submodules for personal-files(&backups) and OS together (for my case i don't use them). each partition should be new GPT or MBR partition. I read many comments, users say btrfs sub-module is complex.
  
- if it is USB drive then choose EXFAT.

  - FAT32 has 4GB file limit. I can not store ISO and VM files.
  - EXFAT is patented. some Linux distro can not read/write via LiveCD.
