...
its Mozilla's official addon which adds additional fetures to default container management. The main feature is that it allows to open spesific domains on spesific containers.
...
(disable search bar on address bar)ozilla's official theme extensions:
...
firefox seperation domains can be managed via native profile or container support. advantages/disadvantages:
